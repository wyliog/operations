...
```shell
kubectl exec -it mongodb-0 -- mongo mydb -u admin -p password --authenticationDatabase admin
```
...
```console
$ kubectl run --attach bbox --image=mongo:3.6 --restart=Never --env="RELEASE_NAME=$RELEASE_NAME" -- sh -c 'while true; do for i in 0 1 2; do echo $RELEASE_NAME-mongodb-$i $(mongo --host=$RELEASE_NAME-mongodb-$i.$RELEASE_NAME-mongodb --eval="printjson(rs.isMaster())" | grep primary); sleep 1; done; done';
...
```console
$ kubectl delete po -l "app=mongodb,release=$RELEASE_NAME"
...
```sh
kubectl exec -it $RELEASE-mongodb-0 -c bootstrap -- tail -f /work-dir/log.txt
```
